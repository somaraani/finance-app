...
export class TransactionsService {
	private static transformPlaidTransaction(
	...
	private static async getPlaidCategoryMap() {
	...
	static async syncUserTransactions(userId: number) {
		...
		const accountMap = await AccountsService.getPlaidAccountMap(userId);
		const categoryMap = await this.getPlaidCategoryMap();
		...
		await Promise.all(
			tokens.map(async ({ accessToken, id, plaidCursor }) => {
				...
				const newTransactions = accountTransactions.data.added.map((transaction) =>
					this.transformPlaidTransaction(transaction, userId, accountMap, categoryMap)
				);
